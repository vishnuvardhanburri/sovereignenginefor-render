...
if [ "$free_tier_safe" = "true" ]; then
  ...
  export FREE_MAIL_PUMP_DISCOVERY_DEADLINE_MS="$(int_between "${FREE_MAIL_PUMP_DISCOVERY_DEADLINE_MS:-}" 45000 15000 60000)"
  export FREE_MAIL_PUMP_APPROVAL_DEADLINE_MS="$(int_between "${FREE_MAIL_PUMP_APPROVAL_DEADLINE_MS:-}" 30000 8000 60000)"
...
if enabled_flag "${WEB_EMBED_FREE_MAIL_PUMP:-$free_mail_pump_default}"; then
  ...
  start_supervised_background "free-mail-pump" env \
    ...
    FREE_MAIL_PUMP_DISCOVERY_DEADLINE_MS="${FREE_MAIL_PUMP_DISCOVERY_DEADLINE_MS:-45000}" \
    FREE_MAIL_PUMP_APPROVAL_DEADLINE_MS="${FREE_MAIL_PUMP_APPROVAL_DEADLINE_MS:-30000}" \
