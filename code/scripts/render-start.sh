...
if [ "$free_tier_safe" = "true" ]; then
  ...
  export DAILY_OUTBOUND_SMALL_MAX_PUBLIC_SEARCH_LIMIT="${DAILY_OUTBOUND_SMALL_MAX_PUBLIC_SEARCH_LIMIT:-3}"
  export DAILY_OUTBOUND_SMALL_MAX_LEAD_SCOUT_LIMIT="${DAILY_OUTBOUND_SMALL_MAX_LEAD_SCOUT_LIMIT:-3}"
fi
...
if enabled_flag "${WEB_EMBED_FREE_MAIL_PUMP:-$free_mail_pump_default}"; then
  ...
  start_background "free-mail-pump" env \
    ...
    NODE_OPTIONS="${FREE_MAIL_PUMP_NODE_OPTIONS:---max-old-space-size=48}" \
